"""
pipeline2.py — Experimental creative pipeline (self-contained).
...
Pipeline shape:
  START
    └── [fn] p2_load_context         ← loads brand guidelines + resolves URIs
          └── p2_culture_researcher   ← Google Search on the demographic
                └── p2_brand_summariser ← reads guidelines, outputs 5-point brand locks
                      └── p2_creative_director ← Big Idea + simplified creative strategy
                            └── p2_prompt_agent  ← converts strategy to image prompt
                                  └── [fn] p2_generate_image ← multimodal Gemini image gen
                                              (product images + reference ads, no layout constraints)

Input (JSON string or dict passed as the initial message):
  {
    "brand":    "Rnorr",
    "audience": "Home cooks aged 25–40, urban UK, time-poor, food-curious",
    "products": ["gs://bucket/brands/Rnorr/Products/stock_pot.png"],
    "assets":   ["gs://bucket/brands/Rnorr/Assets/banner2.jpg"]
  }

Output:
  State key  experiment_image_key  → artifact filename of the generated image
"""
...
def p2_load_context(node_input) -> Event:
    ...
    # Load brand guidelines (same loader used by main pipeline)
    brand_guidelines = ""
...
async def p2_generate_image(
    ...
    """
    Multimodal image generation — no layout constraints, fully creative.

    Sends to Gemini:
      [reference ad images...]  ← style and mood reference
      [product images...]       ← subject to feature
      [prompt]                  ← creative direction from p2_prompt_agent
    """
    ...
    # Text prompt first — Gemini image models need the instruction at the top
    contents.append(p2_image_prompt)

    # Logo — place first so the model treats it as a hard brand constraint
    if p2_logo_uri:
        logo_data = _load_bytes(p2_logo_uri)
        if logo_data:
            contents.append(
                "Brand logo below — reproduce this logo exactly as shown, "
    ...
    for uri in assets:
        ...
        if data:
            contents.append(
                "Reference ad below — a past successful campaign for this brand. "
    ...
    for uri in products:
        ...
        if data:
            contents.append(
                "Product image below — feature this product prominently in the ad."
            )
            contents.append(types.Part.from_bytes(data=data, mime_type=_mime_for(uri)))
...
""",
    ...
    # multi-turn to allow several search calls
)
...
Keep the prompt under 400 words. Output ONLY the prompt — no headers, no explanation.
""",
...
async def p2_adapt_instagram(
    ...
    # Logo first — hard brand constraint
    if p2_logo_uri:
        logo_data = _load_bytes(p2_logo_uri)
        if logo_data:
            contents.append(
                "Brand logo below — reproduce this logo exactly as shown in the adapted image."
            )
            contents.append(
                types.Part.from_bytes(data=logo_data, mime_type=_mime_for(p2_logo_uri))
            )
...
async def p2_adapt_web(
    ...
    """
    Image-to-image adaptation for a landscape web banner (1200×628 / 1.91:1).
    Loads the key visual artifact and re-renders it via Nano Banana 2
    ...
    channel_instruction = (
        ...
        "landscape web banner (1.91:1 ratio, equivalent to 1200×628 px). "
        ...
        "The composition must read clearly at small sizes (thumbnails ~300 px wide) — "
...
async def p2_adapt_mobile(
    ...
    """
    Image-to-image adaptation for a mobile banner (320×50 / 6.4:1).
    Strips the composition down to a single bold graphic element — product
    centred, almost no background detail — readable at thumbnail scale.
    """
    from google import genai as _genai
    ...
    channel_instruction = (
        ...
        "to 320\u00d750 px — the standard mobile leaderboard). "
...
- BODY: 3 short paragraphs — hook, product benefit, call to action
...
async def p2_podcast_tts(
    ...
    # Extract spoken text — drop the SCRIPT: label, READ TIME: trailer,
...
- Sound conversational — written for the ear, not the eye
...
BEAT 1 (0–3s): VISUAL — what the viewer sees. AUDIO — music/SFX/VO line if any.
BEAT 2 (3–6s): VISUAL — the product moment. AUDIO — key message or sound design.
BEAT 3 (6–8s): VISUAL — brand lock-up / CTA. AUDIO — sign-off or music sting.

Then add:
OVERALL MOOD: one sentence
ON-SCREEN TEXT: any supers or captions, listed per beat

Be concrete and directable — a production team should be able to shoot from this.
""",
...
async def p2_campaign_report(
    ...
    """
    Join node — waits for all six channel branches, then aggregates every
...
<p class="meta">Generated by experiment_pipeline · {p2_brand}</p>
...
experiment_pipeline = Workflow(
    ...
    edges=[
        # Linear creative pipeline → key visual → channel router
        (
            "START",
            p2_load_context,
        ...
        # Podcast branch: script → TTS render
...
# ADK web entry point — expose as root_agent so `adk web .` picks it up
