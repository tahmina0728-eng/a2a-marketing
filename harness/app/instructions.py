...
Describe the TEXT ZONE so the composition naturally accommodates it:
  ...
  Achieve this through NATURAL SCENE ELEMENTS only — NOT a fake dark panel:
    - Atmospheric depth: bokeh, natural haze, out-of-focus background
    - Scene lighting that naturally falls off on the left (window light from right, etc.)
    - Open sky, soft wall, or architectural negative space on the left
  The left zone must feel like a deliberate compositional choice, not a shadow bolted on.
  NO faces, products, or busy detail in this zone — those belong in the RIGHT 60%.
  The person and products face LEFT slightly, creating visual tension toward the text zone.
...
Before writing the JSON, ask yourself:
  ...
  ✓ Does the left third have LOW visual complexity — clean enough for large text overlay?
  ✓ Is the clean zone achieved naturally (depth, lighting, space) — not a dark artificial panel?
